...
success: function(data) {
    ...
    for (var i=0; i<data.length; i++) {
        ...
        var row = $('<tr><td><b><img src="http://localhost/web_dev_3/public/img/avatar.jpg" width="30px" height="30px" />' + data[i].nom + ' :<i> '+ data[i].date_post + ':</i></b></br><p style="padding-left:80px">' + data[i].post + '</br><a data-toggle="modal" data-id="'+ commentId +'" title="Add this item" class="open-ReplyModal" href="#ReplyModal">Reply</a>'+'</p></td></tr>');
        $('#record').append(row);
        for (var r = 0; (r < data.length); r++)
                {
                    if ( data[r].parent_comment == commentId)
                    {
                        var comments = $('<tr><td style="padding-left:80px"><b><img src="http://localhost/web_dev_3/public/img/avatar.jpg" width="30px" height="30px" />' + data[r].nom + ' :<i> ' + data[r].date_post + ':</i></b></br><p style="padding-left:40px">'+ data[r].post +'</p></td></tr>');
